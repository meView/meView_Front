...
function AnswerStrength(props) {
  ...
  /* 칩 선택 상태 확인 */
  const [comm, setComm] = useState(answer.strength.includes('communication') ? '' : 'un');
  const [ob, setOb] = useState(answer.strength.includes('observation') ? '' : 'un');
  const [fr, setFr] = useState(answer.strength.includes('friendliness') ? '' : 'un');
  const [lis, setLis] = useState(answer.strength.includes('listening') ? '' : 'un');
  const [exe, setExe] = useState(answer.strength.includes('execution') ? '' : 'un');
  const [jud, setJud] = useState(answer.strength.includes('judgement') ? '' : 'un');
  const [per, setPer] = useState(answer.strength.includes('perseverance') ? '' : 'un');
  /* 선택 칩 개수 확인 */
  let count = 0;
  useEffect(()=>{
  ...
  return (
    <Container>
      ...
      <Bottom>
        <NavigateBtn isNextDisabled={answer.strength.length === 0}/>
      </Bottom>
    </Container>
  )
}
